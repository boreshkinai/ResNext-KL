...
def get_architechture(flags):
    # ResNeXt. total layers = 1 + 3n + 3n + 3n +1 = 9n + 2
    architecture_resnext = [
        ConvLayer(num_outputs=64, kernel_size=3, stride=1, scope='conv0',
                  weights_initializer=ScaledVarianceRandomNormal(),
                  weights_regularizer=tf.contrib.layers.l2_regularizer(scale=WEIGHT_DECAY)),
        ResNextGroup(scope='bottleneck_group1', num_blocks=3, bottleneck_depth=64, cardinality=flags.cardinality, num_filters=64, stride=1),
        ResNextGroup(scope='bottleneck_group2', num_blocks=3, bottleneck_depth=64, cardinality=flags.cardinality, num_filters=128, stride=2),
        ResNextGroup(scope='bottleneck_group3', num_blocks=3, bottleneck_depth=64, cardinality=flags.cardinality, num_filters=256, stride=2),
...
def build_graph(images, flags, is_training):
    conv2d_arg_scope, dropout_arg_scope = _get_scope(is_training)
    architechture = get_architechture(flags)
    ...
    with tf.variable_scope('Model'):
        ...
        with conv2d_arg_scope, dropout_arg_scope:
            ...
            flat_logits = slim.flatten(h, scope='logits/flat_logits')
            h_stack.append(flat_logits)
        return h_stack[-1]
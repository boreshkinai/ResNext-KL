...
# The URL where the CIFAR10 data can be downloaded.
            ...
            for s in range(c_num_shards):
                ...
                with tf.python_io.TFRecordWriter(shard) as tfrecord_writer:
                    ...
                    for i in range(i_min, min(c_num_images, i_max)):
                        sys.stdout.write(
                            '\r>> Building class {}/{}, shard {}/{}, image {}/{}'.format(
                                c, len(_CLASS_NAMES), s, c_num_shards, i, c_num_images))
                        sys.stdout.flush()

                        image = np.squeeze(c_images[i]).transpose((1, 2, 0))
                        label = c_labels[i]

                        png_string = session.run(encoded_image, feed_dict={image_placeholder: image})
